defmodule Strava.RequestBuilder do
  @moduledoc """
  ...
  """
  @spec decode({:ok, Tesla.Env.t()} | {:error, term()}, false | struct() | [struct()]) ::
          {:ok, struct()} | {:error, Tesla.Env.t()} | {:error, term()}
  def decode({:ok, %Tesla.Env{status: 200} = env}, false), do: {:ok, env}

  def decode({:ok, %Tesla.Env{status: 200, body: body}}, struct) do
    options = %{:as => struct}
  ...
  def decode({:ok, %Tesla.Env{} = env}, _struct), do: {:error, env}

  def decode({:error, error}, _struct), do: {:error, error}
end
