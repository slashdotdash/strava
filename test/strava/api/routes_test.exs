  ...
  describe "routes" do
    test "#get_route_by_id" do
      use_cassette "routes/get_route_by_id", match_requests_on: [:query] do
        ...
        assert {:ok, %Strava.Route{} = route} = Strava.Routes.get_route_by_id(client, 1_119_914)
        ...
        assert route.timestamp == 1_414_868_960
        assert route.athlete.id == 784_663
        assert route.map.id == "r1119914"

        assert route.map.polyline ==
        ...
        assert route.map.summary_polyline ==
    ...
    test "#get_routes_by_athlete_id" do
      use_cassette "routes/get_routes_by_athlete_id", match_requests_on: [:query] do
        ...
        assert {:ok, routes} = Strava.Routes.get_routes_by_athlete_id(client, athlete_id)
